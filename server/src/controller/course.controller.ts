...
export const createCourse = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (req.user!.role !== 'teacher' && req.user!.role !== 'admin') {
      res.status(403).json({ message: 'Only teachers and admins can create courses' });
      return;
    }
...
export const getInstructorCourses = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    // Check if user is a teacher or admin
    if (req.user!.role !== 'teacher' && req.user!.role !== 'admin') {
      res.status(403).json({ message: 'Only teachers and admins can view instructor courses' });
      return;
    }
